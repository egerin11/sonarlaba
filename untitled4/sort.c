...
int countIdentical(const struct Table *table, const int pos, const int number,
                   int (*cmp)(const struct Table, const struct Table)) {
    int count = 0;
...
void sort(struct Table **table, int number, int *pos, int *count, enum SortParam sortParam) {
    ...
    switch (sortParam) {
        ...
        case sortByMatrix: {
            ...
            *count = countIdentical(*table, *pos, number,
                                    (int (*)(const struct Table, const struct Table)) cmpByMatrix);
        }
            break;
        case sortByScreen: {
            ...
            *count = countIdentical(*table, *pos, number,
                                    (int (*)(const struct Table, const struct Table)) compareScreen);
        }
            break;
        case sortByPrice : {
            ...
            *count = countIdentical(*table, *pos, number,
                                    (int (*)(const struct Table, const struct Table)) comparePrice);
        }
            break;
        case sortByRam: {
